...
long read_long(const char* buffer, int* delta)
{
    ...
    // sorted sets
    int SimpleRedisClient::zadd(const char *key, int score, const char *member)
    {
        return redis_send( RC_INLINE, "ZADD '%s' %d '%s'\r\n", key, score, member);
    }

    int SimpleRedisClient::zrem(const char *key, const char *member)
    {
        return redis_send( RC_INT, "ZREM '%s' '%s'\r\n", key, member);
    }

    char* SimpleRedisClient::zscore(const char *key, const char *member)
    {
        redis_send( RC_BULK, "ZSCORE '%s' '%s'\r\n", key, member);
        return getData();
    }

    char** SimpleRedisClient::zrevrange(const char *key, int start, int end)
    {
        redis_send( RC_MULTIBULK, "ZREVRANGE '%s' %d %d 'WITHSCORES'\r\n", key, start, end);
        return getMultiBulkData();
    }

    char* SimpleRedisClient::zrevrank(const char *key, const char *member)
    {
        redis_send( RC_INT, "ZREVRANK '%s' '%s'\r\n", key, member);
        return getData();
    }
